...
export default function CardsPage() {
  ...
  const itemsPerView = 2; // Cantidad de botones visibles
  ...
  return (
    <div className="container mx-auto pt-48 px-4 py-8 bg-slate-50">
      <Header />
      <h2 className="text-5xl font-geometos text-[#5D60a6] mb-6 text-center">Selecciona tu memoria</h2>
      ...
      <div className="flex items-center justify-center gap-4 mb-8">
        ...
        <div className="overflow-hidden w-3/5">
          <div className="flex transition-transform duration-500 ease-in-out flex space-x-4" style={{ transform: `translateX(${offset}%)` }}>
            {categories.map((category) => (
              <button
                key={category}
                onClick={() => setActiveCategory(category)}
                className={`px-4 py-2 rounded-lg font-geometos text-white transition-colors  ${activeCategory === category ? 'bg-[#04d9b2]' : 'bg-[#5D60a6] hover:bg-[#04d9b2]'
                  }`}
              >
                {category}
              </button>
      ...
      */}
      ...
      {showModal && selectedTemplate && (
        <div className="fixed inset-0 bg-black bg-slate-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="relative bg-white p-4 sm:p-6 md:p-8 rounded-lg shadow-lg w-full max-w-[95%] sm:max-w-2xl max-h-[95vh] overflow-y-auto">
            ...
            <h2 className="text-3xl font-geometos text-[#5D60a6] mb-4">{selectedTemplate.id}</h2>
