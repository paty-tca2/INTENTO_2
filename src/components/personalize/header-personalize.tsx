...
interface PersonalizeHeaderProps {
  onAddToBasket: (template: CardTemplate | null) => void;
...
const PersonalizeHeader: React.FC<PersonalizeHeaderProps> = ({ onAddToBasket, template }) => {
  ...
  return (
    <>
      ...
      <header className="bg-cyan-500 p-2 fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-2 py-4 md:px-4 md:py-4 bg-white shadow-md">
        ...
        <div className="container mx-auto flex flex-row-reverse md:flex-row justify-between items-center">
          ...
          <div className="flex space-x-2 md:space-x-4 order-1 md:order-2">
            ...
            {/*<button onClick={() => onAddToBasket(template)} className="bg-[#04d9b2] hover:bg-[#5D60a6] text-white px-2 py-1 md:px-4 md:py-2 rounded-full font-geometos flex items-center text-sm md:text-base">
              Añadir al carrito
            </button>*/}
          </div>
        </div>
      </header>
      {/* VENTANA DE PREVIEW DE LAS TARJETAS */}
      {template && (
